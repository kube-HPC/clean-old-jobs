...
const normalizeBuilderPods = (podsRaw) => {
    if (!podsRaw || !podsRaw.body || !podsRaw.body.items) {
        return [];
    }
    const jobs = podsRaw.body.items.map((j) => {
        let ageHours = 0;
        const completed = j.status.phase === 'Succeeded';
        const failed = j.status.containerStatuses.some(c => {
            return c.state.terminated && c.state.terminated.reason === 'Error';
        });

        ageHours = moment().diff(moment(j.status.startTime), 'hours', true);
        return {
            name: j.metadata.labels['job-name'],
            active: j.status.active === 1,
            failed: !!failed,
            completed: !!completed,
            ageHours,

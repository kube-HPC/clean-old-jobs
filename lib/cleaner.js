...
const { normalizeJobs, normalizeWorkerJobs, normalizeBuilderPods } = require('./helpers/normalize');
...
const cleanAlgorithmBuilders = async (options) => {
    const jobsRaw = await kubernetes.getAlgorithmBuilderPods();
    const jobs = normalizeBuilderPods(jobsRaw);
